...
$('#daily-breakfast-table').on('click', '.delete-btn', function(row){
  var rowIndex = $(this).parent().parent()[0].rowIndex;
  var name = $('#daily-breakfast-table')[0].rows[rowIndex].cells[0].innerHTML;
  var calories = $('#daily-breakfast-table')[0].rows[rowIndex].cells[1].innerHTML;

  var storage = new Storage('daily-breakfast', name, calories);
  storage.delete();

  $('#daily-breakfast-table')[0].deleteRow(rowIndex);
  addCalories('daily-breakfast-body', 'breakfast-total-calories');
});
...
function updateRemaining(meal, calories){
  ...
  if (newRemaining < 0){
    ...
    $(`#${meal}-remaining-calories`).addClass('green-text');
  }
...
function totalCaloriesRemaining(){
  ...
  if (totalRemaining < 0){
    ...
    $('#total-remaining-calories').addClass('green-text');
  }
