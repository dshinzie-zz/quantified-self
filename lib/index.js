...
$('#add-selected-exercise').on('click', function(){
  var checkedExercises = $('#diary-exercise-body > tr').filter(':has(:checkbox:checked)').find('td').filter(':nth-child(1), :nth-child(2)')
  $('#daily-exercise-body > tr:first').before(checkedExercises.clone())
})
