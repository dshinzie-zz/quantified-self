...
$('#add-breakfast').on('click', function(){
  ...
  $('#breakfast-body > tr:first').before(newRow);
  $('#breakfast-table tbody tr td input').remove();
  clearCheckboxes('food-checkbox');
  addCalories('breakfast-total-calories');
})
...
$('#add-lunch').on('click', function(){
  ...
  $('#lunch-body > tr:first').before(newRow);
  $('#lunch-table tbody tr td input').remove();
  clearCheckboxes('food-checkbox');
})
...
$('#add-dinner').on('click', function(){
  ...
  $('#dinner-body > tr:first').before(newRow);
  $('#dinner-table tbody tr td input').remove();
  clearCheckboxes('food-checkbox');
})
...
$('#add-snack').on('click', function(){
  ...
  $('#snack-body > tr:first').before(newRow);
  $('#snack-table tbody tr td input').remove();
  clearCheckboxes('food-checkbox');
})
...
function addCalories(calorieClass){
  var sum = 0;

  var cells = document.querySelectorAll("#breakfast-table tbody tr td:nth-of-type(2)");

  for (var i = 0; i < cells.length; i++) {
      sum+=parseFloat(cells[i].firstChild.data);
  }
  document.getElementById('breakfast-total-calories').innerHTML = sum;
  // $('#breakfast-total-calories').append(sum);
}
